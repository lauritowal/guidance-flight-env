...
import guidance_flight_env
import guidance_flight_env.utils.vector2d_old
...
class Simulation(object):
    ...
    # TODO: Use get_property_catalog instead?
    def __init__(self,
    ...
    def get_turn_rate(self):
        ...
        true_airspeed = self.get_true_air_speed()
        bank_angle_rad = self[prp.roll_rad]

        return (bank_angle_rad * gravitational_force) / true_airspeed
    ...
    def get_initial_aircraft_position(self):
        return GeoPosition(longitude_deg=self[prp.initial_longitude_geoc_deg],
                           latitude_deg=self[prp.initial_latitude_geod_deg])

    def is_aircraft_at_target(self, min_distance_to_target: float, target_position: GeoPosition):
        return self.aircraft_distance_to_target_km(target_position=target_position) < min_distance_to_target

    def aircraft_distance_to_target_km(self, target_position):
        aircraft_position = self.get_aircraft_geo_position()
        return aircraft_position.distance_haversine_km(target_position)

    def is_aircraft_out_of_bounds(self, max_distance_km) -> bool:
        current_distance_km = self[prp.dist_travel_m] / 1000
        return current_distance_km > max_distance_km

    def is_aircraft_altitude_to_low(self, min_height_ft) -> bool:
        is_aircraft_altitude_to_low = self[prp.altitude_sl_ft] < min_height_ft
        return is_aircraft_altitude_to_low
    ...
    def get_true_air_speed(self):
        return np.sqrt(np.square(self[prp.u_fps]) + np.square(self[prp.v_fps]) + np.square(self[prp.w_fps]))

    def get_ground_speed(self):
        return np.sqrt(np.square(self[prp.v_north_fps]) + np.square(self[prp.v_east_fps]))

    def get_heading_true_deg(self):
        return (math.degrees(self[prp.heading_true_rad]) - self.offset) % 360

    def get_aircraft_geo_position(self):
        return guidance_flight_env.utils.geoposition.GeoPosition.from_sim(self)

    def get_aircraft_initial_geo_position(self):
        return guidance_flight_env.utils.geoposition.GeoPosition(self[prp.initial_latitude_geod_deg],
                                                        self[prp.initial_longitude_geoc_deg])

    def get_drift_angle_deg(self, track_angle_deg, heading_true_deg):
        return (track_angle_deg - heading_true_deg) % 360

    def calculate_max_distance_km(self, episode_time_s):
        max_distance = self.aircraft.get_max_distance_m(episode_time_s=episode_time_s)
        return max_distance / 1000  # in kilometers

    def get_relative_bearing_to_target_deg(self, target_position):
        aircraft_position = self.get_aircraft_geo_position()
        return aircraft_position.true_bearing_deg_to(target_position)
